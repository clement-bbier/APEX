...
demande un module neuf `services/quant_analytics/core/math/labeling.py`
...
| [services/fusion_engine/meta_labeler.py](../../services/fusion_engine/meta_labeler.py) | `MetaLabeler` déterministe (scoring rules-based, pas un labeler). | **Ne pas toucher** (spec §2.4 : `services/fusion_engine/` frozen). |
| [services/fusion_engine/feature_logger.py](../../services/fusion_engine/feature_logger.py) | Logger produisant un dataset pour MetaLabeler v2. | **Ne pas toucher**. |
...
| [services/quant_analytics/core/math/labeling.py](../../services/quant_analytics/core/math/) | **N'existe pas.** Le dossier `services/quant_analytics/core/math/` n'existe pas. Seul `services/quant_analytics/core/` existe (`service.py`, `realized_vol.py`, `regime_ml.py`, …). | Le chemin mentionné par le prompt de mission est **absent du repo**. |
...
Le prompt de mission demande `services/quant_analytics/core/math/labeling.py`
...
- PHASE_4_SPEC §2.4 (« Deliberately NOT modified by Phase 4 ») inclut
  `services/fusion_engine/` (frozen) et par construction n'autorise pas
...
3. L'API publique `BarrierLabel` / `TripleBarrierConfig` /
   ...
   Un refactor casserait ces tests et `services/fusion_engine/feature_logger.py`
...
chemin inexistant (`services/quant_analytics/core/math/labeling.py`),
...
|---|---|---|
| Chemin du module | `services/quant_analytics/core/math/labeling.py` | `features/labeling/` (spec §3.1 + issue #125) ; **jamais sous `services/`** (spec §2.4) |
...
- **Option B** : l'architecte confirme que le prompt de mission supplante
  la triade. Je crée alors `services/quant_analytics/core/math/labeling.py`
